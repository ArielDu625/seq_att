...
def extract_seq_data(data,numsamples=0,fillnum=100):
    ...
    if 1:
        ...
        for i,s in enumerate(seqdata):
            ...
            #seqarr[i, -len(s):] = np.array(s,dtype="int32")
        seqdata=seqarr
...
def test_fn():
    data_dir='./sst'
    fine_grained=1
    data, voc =load_sentiment_treebank(data_dir,fine_grained)
    print("vocab size:", voc.size())

    for k,d in data.iteritems():
        print(k,len(d))

    print("max len:", get_max_len_data(data))
    
    d=data['dev']
    a,b,c,_=extract_seq_data(d[0:1],0)
    print a,b,c
    
    for subtree in a:
        print(" ".join(map(str,subtree)))
        sentence = []
        for idx in subtree:
            if idx != -1:
                word = voc.decode(idx)
